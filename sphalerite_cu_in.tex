...
varies by 0.05 between 773~K and 373~K in Experiment A1). Cu--In short-range
...
box size and sampling depth it does not reach the fully ordered $E_c = 0$ state.
...
tests this directly (Fig.~\ref{fig:ergo}). A trajectory quenched from a random
...
stationary over the final $1.2\times10^4$ sweeps. The plateau is therefore
...
above the annealed one. This is the model's internal signature of metastability:
the nearest-neighbour association equilibrates quickly, while the longer-range
optimisation of the domain interior does not.
...
from 0.0065 to 0.0120 (Table~\ref{tab:573}). The mechanism is transparent once
the ground state of Section~\ref{sec:groundstate} is recalled: perfect charge
compensation requires each cation to retain four homovalent neighbours out of
twelve, and the assumed
$J_{\mathrm{Cu\text{-}Cu}} = J_{\mathrm{In\text{-}In}} = +0.05$~eV penalises
exactly those contacts. The In shell composition of Experiment A1 shows the
consequence directly, with Cu:In~$= 2.68$ against the ideal 2.00
(Section~\ref{sec:clusters}).

This provides a discriminant that does not depend on knowing $J$ in advance. The
two quantities $\RCuIn$ and $\langle \dQ_a^2 \rangle$ respond to $E_c$ in the
same direction but to $J$ in opposite directions. Measurements sensitive to
...
temperature. More seriously, Experiment D shows that the configuration retains a
memory of its \emph{cooling path} --- quenched and slowly cooled states differ by
79~meV per solute atom while being indistinguishable in pair statistics
(Section~\ref{sec:ergodicity}). Any attempt to read formation conditions from
...
Fourth, all calculations are at a single composition,
...
MIT licence and data under CC~BY~4.0.
